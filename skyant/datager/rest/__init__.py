...
you can fast & simple communicate with the endpoint. All objects skyant.datager.* are inherited from
pydantic, so you can use the same BaseModel as API endpoint definition so and as client-side interface.

This solution makes the process of creating a Client-Server application more effective and minimizes a debugging
time.

General:
    The class General provides methods for interacting with any REST API endpoint.

    [_skyant.datager.rest.General references_](references/rest/General.md)
'''
